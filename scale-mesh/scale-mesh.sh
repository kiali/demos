...
while [[ $# -gt 0 ]]; do
  ...
  case $key in
    ...
    -tgs|--traffic-gen-silent)      TG_SILENT="${2}"; shift; shift ;;
...
Valid options:
  ...
  -tgs|--traffic-gen-silent
      When true, the traffic generator will not produce any logs, thus avoiding huge results.json files
      Default: ${DEFAULT_TG_SILENT}
  -v|--versions
